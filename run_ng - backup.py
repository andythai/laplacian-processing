...
def create_source(url, xyz_dim: list, dim_unit: str, xyz_mat: list):
...
if __name__ == '__main__':
    ...
    # Scaling parameters
    x_dim, y_dim, z_dim = [0.00125, 0.00125, 0.05]  # 1250, 1250, 50000 micrometers
    dim_unit = "m"
    ...
    # Otherwise, load up the image locally
        ...
        dimensions = neuroglancer.CoordinateSpace(names=['z', 'y', 'x'], 
                                                  units=[dim_unit, dim_unit, dim_unit], 
    ...
    with viewer.txn() as s:
        ...
        s.layers.append(
            ...
            source=create_source(url, [x_dim, y_dim, z_dim], dim_unit, [x_mat, y_mat, z_mat]),
            layer=image_layer,
            opacity=1.0,
            shader=image_shader(),
        )
        ...
        if points_path:
            s.layers.append(
                ...
                source=create_source("local://annotations", [x_dim, y_dim, z_dim], dim_unit, [x_mat, y_mat, z_mat]),
        ...
        if segmentation_path:
            s.layers.append(
                ...
                source=create_source(segmentation_layer, [x_dim, y_dim, z_dim], dim_unit, [x_mat, y_mat, z_mat]),
