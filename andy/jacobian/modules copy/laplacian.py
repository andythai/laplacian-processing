...
def sliceToSlice3DLaplacian(fixedImage, mpoints, fpoints):
    ...
    # Use mpoints for indices, and fpoints - mpoints for displacement
    mIndices = mpoints[:,0] * ny*nz + mpoints[:,1] * nz + mpoints[:,2]
    mIndices = mIndices.astype(int)
    
    Xcount[mIndices] += 1
    Ycount[mIndices] += 1
    Zcount[mIndices] += 1
    Xd[mIndices] += fpoints[:,0] - mpoints[:,0]
    Yd[mIndices] += fpoints[:,1] - mpoints[:,1]
    Zd[mIndices] += fpoints[:,2] - mpoints[:,2]
