...
public class MyImageReader
{
    ...
    public static double[][][] convertRGBtoLxy(String fileName )
    {
        ...
        if (numbands < 3)
        {
            System.out.println("Warning: Not an RGB image");
            //return null;
        }
        ...
        if (numbands == 1) {
            System.out.println("Image is grayscale. Copying values into the other two channels.");

            // Allocate a 3-channel array
            rasterValues = new double[3][height][width];

            for (int y = 0; y < height; y++) {
                for (int x = 0; x < width; x++) {
                    double value = raster.getSampleDouble(x, y, 0);
                    rasterValues[0][y][x] = value;  // Luminance
                    rasterValues[1][y][x] = value;  // Copy to x channel
                    rasterValues[2][y][x] = value;  // Copy to y channel
                }
            }
        }
    ...
    public static double[][] genMaxValMap(String fileName) throws Exception
    {
        ...
        if (numbands < 3)
        {
            System.out.println("Warning: cannot be extracted: Not an RGB image");
            //return null;
        }
        ...
        for ( int x = 0; x < width; x++ )
            for( int y = 0; y <height; y++ )
            {
                ...
                double CIEZpix = 0.0;
                if (numbands == 1) {
                    CIEXpix = (0.412453*pixelValues[0] + 0.357580*pixelValues[0] + 0.180423*pixelValues[0])/255.0;
                    CIEYpix = (0.212671*pixelValues[0] + 0.715160*pixelValues[0] + 0.072169*pixelValues[0])/255.0;
                    CIEZpix = (0.019334*pixelValues[0] + 0.119193*pixelValues[0] + 0.950227*pixelValues[0])/255.0;
                }
                else {
                    CIEXpix = (0.412453*pixelValues[0] + 0.357580*pixelValues[1] + 0.180423*pixelValues[2])/255.0;
                    CIEYpix = (0.212671*pixelValues[0] + 0.715160*pixelValues[1] + 0.072169*pixelValues[2])/255.0;
                    CIEZpix = (0.019334*pixelValues[0] + 0.119193*pixelValues[1] + 0.950227*pixelValues[2])/255.0;
                }
